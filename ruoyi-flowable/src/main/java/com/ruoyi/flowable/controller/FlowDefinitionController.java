...
public class FlowDefinitionController extends BaseController {
    ...
    @GetMapping(value = "/listWithPermission")
    @ApiOperation(value = "获取用户有权限发起的流程定义列表", response = FlowProcDefDto.class)
    public AjaxResult listWithPermission(@ApiParam(value = "当前页码", required = true) @RequestParam Integer pageNum,
                                         @ApiParam(value = "每页条数", required = true) @RequestParam Integer pageSize,
                                         @ApiParam(value = "流程名称", required = false) @RequestParam(required = false) String name) {
        Long userId = getUserId();
        return AjaxResult.success(flowDefinitionService.listWithPermission(name, pageNum, pageSize, userId));
    }
    ...
    public AjaxResult save(@RequestBody FlowSaveXmlVo vo) {
        ...
        try {
            ...
            Map<String, String> deployInfo = flowDefinitionService.importFile(vo.getName(), vo.getCategory(), in);
            return AjaxResult.success("导入成功", deployInfo);
        } catch (Exception e) {
            log.error("导入失败:", e);
    ...
    public AjaxResult getProcessHistory(@ApiParam(value = "流程标识") @PathVariable(value = "processKey") String processKey) {
        try {
            List<FlowProcDefDto> historyList = flowDefinitionService.getProcessHistory(processKey);
            return AjaxResult.success(historyList);
        } catch (Exception e) {
            log.error("获取流程历史版本失败：", e);
            return AjaxResult.error("获取流程历史版本失败：" + e.getMessage());
        }
    }

    @ApiOperation(value = "删除流程历史版本")
    @Log(title = "删除流程历史版本", businessType = BusinessType.DELETE)
    @DeleteMapping("/history/{deploymentId}")
    public AjaxResult deleteProcessHistory(@ApiParam(value = "部署ID") @PathVariable(value = "deploymentId") String deploymentId) {
        try {
            flowDefinitionService.deleteProcessHistory(deploymentId);
            return AjaxResult.success("删除成功");
        } catch (Exception e) {
            log.error("删除流程历史版本失败：", e);
            return AjaxResult.error("删除流程历史版本失败：" + e.getMessage());
        }
    }

    @ApiOperation(value = "回退到指定版本")
    @Log(title = "流程版本回退", businessType = BusinessType.INSERT)
    @PostMapping("/rollback")
    public AjaxResult rollbackToVersion(@RequestBody Map<String, Object> rollbackData) {
        try {
            String sourceDeploymentId = (String) rollbackData.get("sourceDeploymentId");
            String processKey = (String) rollbackData.get("processKey");
            String processName = (String) rollbackData.get("processName");
            
            FlowProcDefDto result = flowDefinitionService.rollbackToVersion(sourceDeploymentId, processKey, processName);
            return AjaxResult.success("版本回退成功", result);
        } catch (Exception e) {
            log.error("流程版本回退失败：", e);
            return AjaxResult.error("流程版本回退失败：" + e.getMessage());
        }
    }
}
