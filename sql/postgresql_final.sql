...
CREATE OR REPLACE VIEW view_self_table_columns AS
SELECT
    -- 基础信息
    table_catalog,
    table_schema,
    ...
    -- 约束信息
    CASE
        WHEN is_nullable = 'NO' AND c.contype != 'p' THEN '1'
        ELSE NULL
    END AS is_required,
    ...
    -- 注释和类型
    COALESCE(c.DeText, '') AS column_comment,
    COALESCE(c.typname, '') AS column_type,
    COALESCE(c.contype, '') AS contype
...
LEFT JOIN (
    SELECT
        ...
        -- 列信息
        a.attname,
        col_description(c.oid, a.attnum) AS DeText,  -- 正确的列注释函数
        t.typname,
    ...
    LEFT JOIN (
        SELECT
            conrelid,
            unnest(conkey) AS col_num,
            contype
        FROM pg_constraint
        WHERE contype IN ('p', 'u', 'f')
    ) cons ON cons.conrelid = c.oid AND cons.col_num = a.attnum

    WHERE c.relkind = 'r'  -- 只查询表
        AND a.attnum > 0
        AND NOT a.attisdropped
        AND n.nspname NOT IN ('pg_catalog', 'information_schema')
) c ON cols.table_schema = c.nspname
    AND cols.table_name = c.relname
    AND cols.column_name = c.attname
