...
public class FlowTaskServiceImpl extends FlowServiceFactory implements IFlowTaskService {
    ...
    public AjaxResult complete(FlowTaskVo taskVo) {
        ...
        if (DelegationState.PENDING.equals(task.getDelegationState())) {
            ...
            // 根据操作类型动态设置意见类型，默认为通过
            String commentType = getCommentTypeByAction(taskVo.getActionType(), FlowComment.NORMAL);
            taskService.addComment(taskVo.getTaskId(), taskVo.getInstanceId(), commentType, taskVo.getComment());
    ...
    public void taskReject(FlowTaskVo flowTaskVo) {
        ...
        if (task == null) {
            throw new CustomException("任务不存在或已被处理!");
        }
        ...
        // 检查当前节点是否为真正的初始节点（即前面只有开始节点）
        boolean isRealInitialNode = false;
        if (parentUserTaskList == null || parentUserTaskList.size() == 0) {
            // 检查当前节点的前置节点是否直接连接到开始节点
            List<SequenceFlow> incomingFlows = FlowableUtils.getElementIncomingFlows(source);
            if (incomingFlows != null) {
                for (SequenceFlow flow : incomingFlows) {
                    if (flow.getSourceFlowElement() instanceof StartEvent) {
                        // 如果前置节点是开始节点，说明这是第二个节点（第一个用户任务节点），不是初始节点
                        isRealInitialNode = false;
                        break;
                    } else {
                        // 如果前置节点不是开始节点且没有找到父级用户任务，才是真正的初始节点
                        isRealInitialNode = true;
                    }
                }
            } else {
                // 没有入口连线，说明是真正的初始节点
        ...
        if (isRealInitialNode) {
            throw new CustomException("当前节点为初始任务节点，不能驳回");
        }

        // 如果没有找到父级用户任务节点，但前置节点是开始节点，需要特殊处理
        if (parentUserTaskList == null || parentUserTaskList.size() == 0) {
            // 对于第二个节点（第一个用户任务节点），直接结束流程并记录驳回流转记录
            handleFirstUserTaskReject(flowTaskVo, task);
            return;
        }
        // 获取活动 ID 即节点 Key
        ...
        // 设置驳回意见，支持动态操作类型
        String commentType = getCommentTypeByAction(flowTaskVo.getActionType(), FlowComment.REJECT);
        currentTaskIds.forEach(item -> taskService.addComment(item, task.getProcessInstanceId(), commentType, flowTaskVo.getComment()));
    ...
    public void taskReturn(FlowTaskVo flowTaskVo) {
        ...
        // 设置回退意见，支持动态操作类型
        String commentType = getCommentTypeByAction(flowTaskVo.getActionType(), FlowComment.REBACK);
        currentTaskIds.forEach(currentTaskId -> taskService.addComment(currentTaskId, task.getProcessInstanceId(), commentType, flowTaskVo.getComment()));
    ...
    public AjaxResult myProcess(FlowQueryVo queryVo) {
        ...
        for (HistoricProcessInstance hisIns : historicProcessInstances) {
            ...
            ProcessDefinition pd = repositoryService.createProcessDefinitionQuery()
                    ...
                    .singleResult();
            // 按名称搜索
            if (StringUtils.isNotBlank(queryVo.getName()) && Objects.nonNull(pd) && !pd.getName().contains(queryVo.getName())) {
                continue;
            }
...
//        TODO 传入名称查询不到数据?
    ...
    public AjaxResult finishedList(FlowQueryVo queryVo) {
        ...
        for (HistoricTaskInstance histTask : historicTaskInstanceList) {
            ...
            // 检查流程是否已结束，如果已结束且当前任务是最后一个任务，则显示"结束"
            HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery()
                    .processInstanceId(histTask.getProcessInstanceId())
                    .singleResult();

            String taskName = histTask.getName();
            if (historicProcessInstance != null && historicProcessInstance.getEndTime() != null) {
                // 流程已结束，检查是否是最后一个任务
                List<HistoricTaskInstance> allTasksInProcess = historyService.createHistoricTaskInstanceQuery()
                        .processInstanceId(histTask.getProcessInstanceId())
                        .orderByHistoricTaskInstanceEndTime()
                        .desc()
                        .list();

                // 如果当前任务是流程中的最后一个任务，则显示"结束"
                if (!allTasksInProcess.isEmpty() && allTasksInProcess.get(0).getId().equals(histTask.getId())) {
                    // 检查流程实例是否因驳回而结束
                    if (StringUtils.isNotBlank(historicProcessInstance.getDeleteReason())
                            && historicProcessInstance.getDeleteReason().contains("驳回")) {
                        taskName = "驳回结束";
                    } else {
                        taskName = "结束";
                    }
        ...
        if (StringUtils.isNotBlank(procInsId)) {
            ...
            for (HistoricActivityInstance histIns : list) {
                ...
                if ("startEvent".equals(histIns.getActivityType())) {
                    ...
                    flowTask.setAssigneeId(startUser.getUserId());
                    flowTask.setAssigneeName(startUser.getNickName());
                    flowTask.setDeptName(Objects.nonNull(startUser.getDept()) ? startUser.getDept().getDeptName() : "");
                    ...
                    HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery()
                            ...
                            .singleResult();
                    String taskName = "结束";
                    if (historicProcessInstance != null && StringUtils.isNotBlank(historicProcessInstance.getDeleteReason())
                            && historicProcessInstance.getDeleteReason().contains("驳回")) {
                        taskName = "驳回结束";
                    } else if (StringUtils.isNotBlank(histIns.getActivityName())) {
                        taskName = histIns.getActivityName();
                    }
                    flowTask.setTaskName(taskName);
                    flowTask.setCreateTime(histIns.getStartTime());
                    flowTask.setFinishTime(histIns.getEndTime());
                    ...
                    // 查找与当前任务ID匹配的评论，优先获取最新的评论
                    Comment matchedComment = null;
                    for (Comment comment : commentList) {
                        if (histIns.getTaskId().equals(comment.getTaskId())) {
                            // 如果找到匹配的评论，保存它（如果有多个，会保留最后一个，即最新的）
                            matchedComment = comment;
                        }
                    }
                    // 如果找到匹配的评论，设置到flowTask中
                    if (matchedComment != null) {
                        flowTask.setComment(FlowCommentDto.builder()
                                .type(matchedComment.getType())
                                .comment(matchedComment.getFullMessage())
                                .build());
                    }
                    hisFlowList.add(flowTask);
                }
     ...
     */
    @Override
    public AjaxResult processVariables(String taskId,String deployId) {
        // 流程变量
        ...
        if (Objects.nonNull(historicTaskInstance)) {
            ...
            //查询form
            SysForm sysForm = sysInstanceFormService.selectSysDeployFormByDeployId(deployId);
            if (sysForm.getFormContent()==null) {
                variables.put("form",sysForm);
            }
    ...
    public AjaxResult flowFormData(String deployId) {
        ...
        if (StringUtils.isNotBlank(deployId)) {
            ...
            return AjaxResult.success(sysForm);
        } else {
     ...
     */
    @Override
    public AjaxResult flowTaskForm(String taskId,String deployId) {
        Task task = taskService.createTaskQuery().taskId(taskId).singleResult();
        // 流程变量
        ...
        JSONObject formJson = JSONObject.parseObject(JSON.toJSONString(parameters.get("formJson")));
        if (formJson!=null) {
            JSONObject oldVariables;
            if (formJson.getJSONArray("widgetList")==null) {
                oldVariables = formJson.getJSONObject("formContent");
            }else{
                oldVariables = formJson;
            }
...
//        flowTask.setVariables(historicTaskInstance.getProcessVariables());
    ...
    /**
     * 处理第一个用户任务节点驳回
     * 当第一个用户任务节点被驳回时，直接结束流程并设置删除原因
     *
     * @param flowTaskVo 流程任务参数
     * @param task       当前任务
     */
    private void handleFirstUserTaskReject(FlowTaskVo flowTaskVo, Task task) {
        try {
            // 添加驳回意见到任务评论，支持动态操作类型
            String commentType = getCommentTypeByAction(flowTaskVo.getActionType(), FlowComment.REJECT);
            taskService.addComment(task.getId(), task.getProcessInstanceId(),
                    commentType, flowTaskVo.getComment());
            ...
            if (processInstance != null) {
                // 构建删除原因
                String deleteReason = "驳回：" + (StringUtils.isNotBlank(flowTaskVo.getComment()) ?
                        flowTaskVo.getComment() : "第一个用户任务节点驳回，流程结束");

                // 直接删除流程实例，设置删除原因为驳回
                // 注意：不需要先跳转到结束节点再删除，直接删除即可
                runtimeService.deleteProcessInstance(task.getProcessInstanceId(), deleteReason);

                log.info("第一个用户任务节点驳回，流程实例 {} 已删除，删除原因: {}",
                        task.getProcessInstanceId(), deleteReason);
            } else {
                log.warn("流程实例 {} 不存在，可能已被删除", task.getProcessInstanceId());
            }
        } catch (FlowableObjectNotFoundException e) {
            log.warn("流程实例 {} 不存在: {}", task.getProcessInstanceId(), e.getMessage());
        } catch (Exception e) {
            log.error("处理第一个用户任务节点驳回失败: {}", e.getMessage(), e);
            throw new CustomException("处理第一个用户任务节点驳回失败: " + e.getMessage());
        }
    }

    /**
     * 根据操作类型获取对应的意见类型
     * 支持灵活的审批状态记录，便于未来扩展新的操作类型
     *
     * @param actionType     操作类型
     * @param defaultComment 默认意见类型
     * @return 意见类型字符串
     */
    private String getCommentTypeByAction(String actionType, FlowComment defaultComment) {
        if (StringUtils.isBlank(actionType)) {
            return defaultComment.getType();
        }

        // 根据操作类型映射到对应的FlowComment
        switch (actionType.toUpperCase()) {
            case "NORMAL":
                return FlowComment.NORMAL.getType();
            case "REJECT":
                return FlowComment.REJECT.getType();
            case "REBACK":
                return FlowComment.REBACK.getType();
            case "DELEGATE":
                return FlowComment.DELEGATE.getType();
            case "ASSIGN":
                return FlowComment.ASSIGN.getType();
            case "STOP":
                return FlowComment.STOP.getType();
            default:
                // 如果是未知的操作类型，使用默认类型
                log.warn("未知的操作类型: {}，使用默认类型: {}", actionType, defaultComment.getRemark());
                return defaultComment.getType();
        }
    }
}
